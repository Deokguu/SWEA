...
'''
def prim(start_node):
    pq = [(0, start_node)] #우선순위큐. 시작점은 가중치가 0이다
    ...
    while pq:
        ...
        for next_node in range(V):
            # 갈 수 없으면 pass, 이미 방문했으면 pass
            if next_node == 0:
                continue
            if MST[next_node]:
                continue
            heapq.heappush(graph[node][next_node], next_node) #가중치, 노드번호 enque
    return min_weight
...
graph = [[0] * V for _ in range(V)] #인접행렬. 선택과제로 인접리스트로도 해보기
